...
// Fungsi untuk memuat restoran favorit dari IndexedDB
function loadFavorites() {
    openDatabase().then(db => {
        ...
        request.onerror = () => console.error("Failed to load favorites");
    });
}
...
function showFavorites(favorites) {
    ...
    // Buat elemen untuk restoran favorit
    const newSection = document.createElement("section");
    newSection.classList.add("favorites-section");
    newSection.innerHTML = <h2 class="favorites-title">Your Favorite Restaurants</h2>;

    favorites.forEach((restaurant) => {
        const card = document.createElement("div");
        card.classList.add("restaurant-card-fav");
        card.innerHTML = 
            <div class="location-fav">City: ${restaurant.city}</div>
            <img src="https://restaurant-api.dicoding.dev/images/small/${restaurant.pictureId}" alt="${restaurant.name}">
            <div class="restaurant-card__details-fav">
                <p class="rating-fav">Rating: <span>${restaurant.rating}</span></p>
                <h3>${restaurant.name}</h3>
                <p class="description-fav">${restaurant.description.substring(0, 50)}...</p>
                <button class="remove-favorite" data-id="${restaurant.id}">Remove</button>
            </div>
        ;
        newSection.appendChild(card);

        // Tambahkan event listener untuk menghapus dari favorit
    ...
    // Pastikan footer tampil di bawah
    footer.style.display = "block";
    footer.style.position = "relative";
    document.body.appendChild(footer);
}

// Tambahkan event listener untuk menu favorit
